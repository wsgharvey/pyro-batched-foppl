...
from pyro.ops.integrator import _potential_grad

from batch_pyro.utils import flatten, unpack


def velocity_verlet(z, r, potential_fn, inverse_mass_matrix, step_size, batch_size, num_steps=1, z_grads=None):
    r"""
...
def _single_step_verlet(z, r, potential_fn, inverse_mass_matrix, step_size, batch_size, z_grads=None):
    r"""
    ...
    r_grads = _kinetic_grad(inverse_mass_matrix, r, batch_size)
    for site_name in z:
...
def _potential_grad(potential_fn, z):
    ...
    potential_energy = potential_fn(z).sum()
